...
**************************************************************************************
	...
	XDEF		_IKBD_GetHandler
	XDEF		_IKBD_SetHandler

	XDEF		_IKBD_GetMidiBytesWaiting
	XDEF		_IKBD_GetKbdBytesWaiting
	XDEF		_IKBD_PopMidiByte
	XDEF		_IKBD_PopKbdByte

	XDEF		_IKBD_Flush
	XDEF		_IKBD_FlushGemdos
	XDEF		_IKBD_SendCmd

	XDEF		_IKBD_MainHandler
	XDEF		_IKBD_PowerpadHandler
	XDEF		_IKBD_ReadPowerpadA
	XDEF		_IKBD_ReadPowerpadB

	XDEF		_IKBD_ReadPadMatrixA
	XDEF		_IKBD_ReadPadMatrixB

	XDEF		_IKBD_InitTosLink

	XDEF	_IKBD_InstallTrap13
	XDEF	_IKBD_RestoreTrap13
...
*------------------------------------------------------------------------------------*
* FUNCTION : void (*_IKBD_GetHandler())()
...
*------------------------------------------------------------------------------------*
* FUNCTION : void _IKBD_SetHandler( void (*apHandler)() )
* ACTION   : returns pointer to current ikbd handler
* CREATION : 25.03.00 PNK
*------------------------------------------------------------------------------------*
...
*------------------------------------------------------------------------------------*
* FUNCTION : U16 _IKBD_GetMidiBytesWaiting()
* ACTION   : gets numbers of MIDI bytes waiting in buffer
* CREATION : 13.04.99 PNK
*------------------------------------------------------------------------------------*

_IKBD_GetMidiBytesWaiting:

	move.w	_gIKBD+sIKBD_MidiTail,d0		; tail position
	sub.w	_gIKBD+sIKBD_MidiHead,d0		; -head position = size of queue
...
_IKBD_PopMidiByte:
	...
	move.w	_gIKBD+sIKBD_MidiHead,d1		; current head index
	lea		_gIKBD+sIKBD_MidiBuffer,a0	; midi buffer
	move.b	(a0,d1.w),d0				; retrieve byte from buffer
	addq.w	#1,d1						; point head to next
	and.w	#2047,d1					; ensure cyclical buffer
	move.w	d1,_gIKBD+sIKBD_MidiHead		; update head index
...
*------------------------------------------------------------------------------------*
* FUNCTION : U16 _IKBD_GetKbdBytesWaiting()
* ACTION   : gets numbers of IKBD bytes waiting in buffer
* CREATION : 13.04.99 PNK
*------------------------------------------------------------------------------------*

_IKBD_GetKbdBytesWaiting:

	move.w	_gIKBD+sIKBD_KbdTail,d0	; tail position
	sub.w	_gIKBD+sIKBD_KbdHead,d0	; -head position = size of queue
	bpl.s	.plus					; no need to cater for cyclic buffer
	add.w	#2048,d0				; absolute queue size
.plus:
...
_IKBD_PopKbdByte:
	...
	move.w	_gIKBD+sIKBD_KbdHead,d1		; current head index
	lea		_gIKBD+sIKBD_KbdBuffer,a0	; midi buffer
	move.b	(a0,d1.w),d0				; retrieve byte from buffer
	addq.w	#1,d1						; point head to next
	and.w	#2047,d1					; ensure cyclical buffer
	move.w	d1,_gIKBD+sIKBD_KbdHead		; update head index
...
*------------------------------------------------------------------------------------*
* FUNCTION : void _IKBD_FlushGemdos()
...
*------------------------------------------------------------------------------------*
* FUNCTION : void _IKBD_SendCmd( U8 aCmd )
...
*------------------------------------------------------------------------------------*
* FUNCTION : void _IKBD_MainHandler()
...
_IKBD_MainHandler:
	...
	lea		_gIKBD,a3				; ikbd data structure
.int_loop:
...
*------------------------------------------------------------------------------------*
* FUNCTION : void _IKBD_InitTosLink
...
IKBD_Bconstat:
	...
	lea		_gIKBD+sIKBD_KbdBuffer,a0	; our keyboard ring buffer

	move.w	_gIKBD+sIKBD_KbdHead,d1		; head offset into ring buffer
.loop:
	and.w	#2047,d1					; wrap by ring buffer size
	cmp.w	_gIKBD+sIKBD_KbdTail,d1		; have we reached tail?
...
.next:
	moveq	#0,d0							; clear d0
	bsr		_IKBD_GetKbdBytesWaiting			; determine if there are bytes waiting
	tst.l	d0								; check result
	beq.s	.none							; nothing to process

	moveq	#0,d0							; clear d0
	bsr		_IKBD_PopKbdByte					; grab keyboard byte
...
.some:
	move.l	_gIKBD+sIKBD_pUnshiftTable,a0	; standard keyboard table
	tst.b	_gIKBD+sIKBD_KbdPressTable+$2a	; is left shift pressed?
	bne.s	.shift							; yes, use shift table
	tst.b	_gIKBD+sIKBD_KbdPressTable+$36	; is right shift pressed?
	beq.s	.noshift						; no, use unshifted table
.shift:
	move.l	_gIKBD+sIKBD_pShiftTable,a0		; shifted table
.noshift:
	moveq	#0,d1							; clear d1
	move.b	(a0,d0.w),d1					; read ASCII value of key from table
	tst.b	_gIKBD+sIKBD_KbdPressTable+$1d	; is control pressed?
	beq.s	.notControl						; nope
	and.w	#$1f,d1							; mangle ASCII for control 
.notControl:
	tst.b	_gIKBD+sIKBD_KbdPressTable+$38	; is ALT pressed?
	beq.s	.notALT							; nope
...
.write:
	clr.b	_gIKBD+sIKBD_KbdPressTable+$36		; clear right shift status
	btst	#0,d0								; should we set right shift?
	bne.s	.noShiftRight						; no
	st.b	_gIKBD+sIKBD_KbdPressTable+$36		; set right shift status
.noShiftRight:
	clr.b	_gIKBD+sIKBD_KbdPressTable+$2a		; clear left shift status
	btst	#1,d0								; should we set left shift?
	bne.s	.noShiftLeft						; no
	st.b	_gIKBD+sIKBD_KbdPressTable+$2a		; set left shift status
.noShiftLeft:
	clr.b	_gIKBD+sIKBD_KbdPressTable+$1d		; clear control status
	btst	#2,d0								; should we set control?
	bne.s	.noControl							; no
	st.b	_gIKBD+sIKBD_KbdPressTable+$1d		; set control status
.noControl:
	clr.b	_gIKBD+sIKBD_KbdPressTable+$38		; clear alt status
	btst	#3,d0								; should we set alt?
	bne.s	.noALT								; no
	st.b	_gIKBD+sIKBD_KbdPressTable+$38		; set alt status
.noALT:
	clr.b	_gIKBD+sIKBD_KbdPressTable+$3A		; clear capslock status
	btst	#4,d0								; should we set capslock?
	bne.s	.noCapsLock							; no
	st.b	_gIKBD+sIKBD_KbdPressTable+$3A		; set capslock status
.noCapsLock:
	clr.b	_gIKBD+sIKBD_KbdPressTable+$47		; clear clr/home status
	btst	#5,d0								; should we set clr/home shift?
	bne.s	.noClrHome							; no
	st.b	_gIKBD+sIKBD_KbdPressTable+$47		; set clr/home status
.noClrHome:
	clr.b	_gIKBD+sIKBD_KbdPressTable+$52		; clear insert status
	btst	#6,d0								; should we set insert?
	bne.s	.noInsert							; no
	st.b	_gIKBD+sIKBD_KbdPressTable+$52		; set insert status
.noInsert:
...
IKBD_Kbshift_Build:
	moveq	#0,d0								; clear d0
	tst.b	_gIKBD+sIKBD_KbdPressTable+$36		; is right shift set?
	beq.s	.blankShiftRight					; not set
	bset	#0,d0								; set relevant bit
.blankShiftRight:
	tst.b	_gIKBD+sIKBD_KbdPressTable+$2a		; is left shift set?
	beq.s	.blankShiftLeft						; not set
	bset	#1,d0								; set relevant bit
.blankShiftLeft:
	tst.b	_gIKBD+sIKBD_KbdPressTable+$1d		; is contrl set?
	beq.s	.blankControl						; not set
	bset	#2,d0								; set relevant bit
.blankControl:
	tst.b	_gIKBD+sIKBD_KbdPressTable+$38		; is alt set?
	beq.s	.blankALT							; not set
	bset	#3,d0								; set relevant bit
.blankALT:
	tst.b	_gIKBD+sIKBD_KbdPressTable+$3A		; is caps lock set?
	beq.s	.blankCapsLock						; not set
	bset	#4,d0								; set relevant bit
.blankCapsLock:
	tst.b	_gIKBD+sIKBD_KbdPressTable+$47		; is clr/home set?
	beq.s	.blankClrHome						; not set
	bset	#5,d0								; set relevant bit
.blankClrHome:
	tst.b	_gIKBD+sIKBD_KbdPressTable+$52		; is insert set?
...
*------------------------------------------------------------------------------------*
* FUNCTION : void _IKBD_PowerpadHandler()
...
_IKBD_PowerpadHandler:
	movem.l	a0-a2/d0-d3,-(a7)			; save registers

.check0:
	moveq	#1,d0
	and.b	_gIKBD+sIKBD_TeamTapActiveBits,d0
	beq.s	.noTeamTap0

	moveq	#4-1,d0
	lea		gKbdJagPadMasksA,a0
	lea		_gIKBD+sIKBD_TeamTapDirs,a1
	lea		_gIKBD+sIKBD_TeamTapKeys,a2
.loop0:
	bsr		_IKBD_ReadPadMatrixA
	addq.l	#4,a0
	addq.l	#1,a1
	addq.l	#2,a2
	dbra	d0,.loop0
...
.noTeamTap0:
	...
	lea		gKbdJagPadMasksA,a0
	lea		_gIKBD+sIKBD_Pad0Dir,a1
	lea		_gIKBD+sIKBD_Pad0Key,a2
	bsr		_IKBD_ReadPadMatrixA
...
.check1:
	...
	lea		_gIKBD+sIKBD_TeamTapDirs+4,a1
	lea		_gIKBD+sIKBD_TeamTapKeys+8,a2
.loop1:
	bsr		_IKBD_ReadPadMatrixB
	addq.l	#4,a0
	addq.l	#1,a1
	addq.l	#2,a2
	dbra	d0,.loop1
...
.noTeamTap1:
	...
	lea		gKbdJagPadMasksB,a0
	lea		_gIKBD+sIKBD_Pad1Dir,a1
	lea		_gIKBD+sIKBD_Pad1Key,a2
	bsr		_IKBD_ReadPadMatrixB
...
*------------------------------------------------------------------------------------*
* FUNCTION : void _IKBD_ReadPowerpadA()
...
.no_option:
	...
	move.b	d1,_gIKBD+sIKBD_Pad0Dir		; save directional+fire data in variable
	move.w	d2,_gIKBD+sIKBD_Pad0Key		; save key data in variable
...
*------------------------------------------------------------------------------------*
* FUNCTION : void _IKBD_ReadPowerpadB()
...
.no_option:
	...
	move.b	d1,_gIKBD+sIKBD_Pad1Dir		; save directional+fire data in variable
	move.w	d2,_gIKBD+sIKBD_Pad1Key		; save key data in variable
...
*------------------------------------------------------------------------------------*
* FUNCTION : _IKBD_ReadPadMatrixA
* ACTION   : reads teamtap a
* CREATION : 11.11.01 PNK
*------------------------------------------------------------------------------------*

_IKBD_ReadPadMatrixA:
	movem.l	d0-d2/a0,-(a7)
...
*------------------------------------------------------------------------------------*
* FUNCTION : _IKBD_ReadPadMatrixB
* ACTION   : reads teamtap b
* CREATION : 11.11.01 PNK
*------------------------------------------------------------------------------------*

_IKBD_ReadPadMatrixB:
	movem.l	d0-d2/a0,-(a7)
