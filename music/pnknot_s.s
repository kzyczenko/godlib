...
**************************************************************************************
	...
	XDEF	_PinkNote_Player

	XDEF	_PinkNote_NotePlayChannelA
	XDEF	_PinkNote_NotePlayChannelB
	XDEF	_PinkNote_NotePlayChannelC

	XREF	_gPinkNoteChannel
...
*------------------------------------------------------------------------------------*
* FUNCTION : _PinkNote_Player( void )
...
.PlayA:
	lea		_gPinkNoteChannel+(oPinkNoteChannel_sizeof*0),a0		;	base of ChannelA data
	tst.b	oPinkNoteChannel_ActiveFlag(a0)						;	is ChannelA on?
	beq.s	.PlayB												;	no

	bsr		PinkNote_ProcessChannel								;	read data from note stream
	bsr		_PinkNote_NotePlayChannelA							;	play sounds on ChannelA

.PlayB:
	lea		_gPinkNoteChannel+(oPinkNoteChannel_sizeof*1),a0		;	base of ChannelB data
	tst.b	oPinkNoteChannel_ActiveFlag(a0)						;	is ChannelB on?
	beq.s	.PlayC												;	no

	bsr		PinkNote_ProcessChannel								;	read data from note stream
	bsr		_PinkNote_NotePlayChannelB							;	play sounds on ChannelB

.PlayC:
	lea		_gPinkNoteChannel+(oPinkNoteChannel_sizeof*2),a0		;	base of ChannelC data
	tst.b	oPinkNoteChannel_ActiveFlag(a0)						;	is ChannelC on?
	beq.s	.PlayRts											;	no

	bsr		PinkNote_ProcessChannel								;	read data from note stream
	bsr		_PinkNote_NotePlayChannelC							;	play sounds on ChannelC
...
*------------------------------------------------------------------------------------*
* FUNCTION : _PinkNote_NotePlayChannelA( U32 * apNote )
* ACTION   : plays a note on channel A
* CREATION : 04.02.01 PNK
*------------------------------------------------------------------------------------*

_PinkNote_NotePlayChannelA:
...
*------------------------------------------------------------------------------------*
* FUNCTION : _PinkNote_NotePlayChannelB( U32 * apNote )
* ACTION   : plays a note on channel B
* CREATION : 04.02.01 PNK
*------------------------------------------------------------------------------------*

_PinkNote_NotePlayChannelB:
