...
**************************************************************************************
	...
	xdef	_AudioMixer_Vbl

	xdef	_gpAudioMixerBuffer
	xdef	_gpAudioMixerSilence
	xdef	_gpAudioMixerMulTable
	xdef	_gAudioMixerLockFlag
	xdef	_gAudioMixerBufferOffset
	xdef	_gAudioMixerSamples
	xdef	_gAudioMixerEnableFlag

	xref	_gAudioMixerSineLaw
...
_AudioMixer_Vbl:
	not.w	$ffff8240.w
	tas		_gAudioMixerLockFlag			; already in a mixer interrupt?
	bne		.locked						; yes, don't try further mixing

	movem.l	d0-a6,-(a7)					; save registers

	lea		_gAudioMixerSamples,a0
	lea		sAmixerSpl_msizeof(a0),a1

	move.l	_gpAudioMixerMulTable,a2
	moveq	#0,d0
...
.mixO:
	...
	move.l	_gpAudioMixerBuffer,a2			; start of mixing buffer
	sub.l	a2,d7							; current h/w offset into buffer
	move.l	_gAudioMixerBufferOffset,d6		; end of last s/w mix
	...
	move.l	d7,_gAudioMixerBufferOffset				; end of new s/w mix
	...
	move.l	_gpAudioMixerBuffer,a2
	move.l	_gAudioMixerBufferOffset,d0
	jsr		(a3)
	bra.s	.update
.lpls:
	move.l	d7,d0
...
.update:
	...
	clr.b	_gAudioMixerLockFlag			; signal end of mixing
...
AudioMixer_UpdateSamples:
	...
	move.l	_gpAudioMixerSilence,a1					; go back to silence sample if queue is empty
	lea		_gAudioMixerSamples,a0					; start of samples
...
_gpAudioMixerBuffer:			dc.l	0
_gpAudioMixerSilence:		dc.l	0
_gpAudioMixerMulTable:		dc.l	0
_gAudioMixerBufferOffset:	dc.l	0
_gAudioMixerLockFlag:		dc.b	0
_gAudioMixerEnableFlag:		dc.b	0
...
_gAudioMixerSamples:			ds.l	(sAmixerSpl_msizeof*eAMIXER_CHANNEL_LIMIT)
